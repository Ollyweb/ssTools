...
if (-not $isAdmin) {
    Write-Host ""
    Write-Host "                     ADMIN PRIVILEGES REQUIRED" -ForegroundColor $C_Error
    Write-Host "                     Please run this script AS AN ADMIN." -ForegroundColor $C_Error
...
try {
    ...
    Write-Host "┌─ SYSTEM BOOT INFO ────────────────────────────────────────────────┐" -ForegroundColor $C_Section
    Write-Host ("  Last Boot : {0}" -f $bootTime.ToString("yyyy-MM-dd HH:mm:ss")) -ForegroundColor $C_Subtext
    Write-Host ("  Uptime    : {0} days, {1:D2}:{2:D2}:{3:D2}" -f $uptime.Days, $uptime.Hours, $uptime.Minutes, $uptime.Seconds) -ForegroundColor $C_Info
    Write-Host "└───────────────────────────────────────────────────────────────────┘" -ForegroundColor $C_Section
} catch {
    Write-Host "Unable to retrieve boot time information" -ForegroundColor $C_Error
}
...
foreach ($svc in $services) {
    ...
    if ($service) {
        ...
        if ($service.Status -eq "Running") {
            Write-Host -NoNewline ("  {0,-12} {1,-40}" -f $svc.Name, $displayName) -ForegroundColor $C_Info
            if ($svc.Name -eq "Bam") {
                Write-Host (" | {0}" -f "Enabled") -ForegroundColor $C_Warn
            } else {
                try {
                    ...
                    if ($process.ProcessId -gt 0) {
                        ...
                        if ($proc) {
                            Write-Host (" | started: {0}" -f $proc.StartTime.ToString("HH:mm:ss")) -ForegroundColor $C_Warn
                        } else {
                            Write-Host " | started: N/A" -ForegroundColor $C_Warn
                        }
                    } else {
                        Write-Host " | started: N/A" -ForegroundColor $C_Warn
                    }
            ...
            Write-Host ("  {0,-12} {1,-40} {2}" -f $svc.Name, $displayName, $service.Status) -ForegroundColor $C_Error
        }
    } else {
...
foreach ($s in $settings) {
    ...
    if ($status -and $status.$($s.Key) -eq 0) {
        Write-Host ("{0}: " -f $s.Name) -NoNewline -ForegroundColor $C_Subtext
        Write-Host $($s.Warning) -ForegroundColor $C_Error
    } else {
        Write-Host ("{0}: " -f $s.Name) -NoNewline -ForegroundColor $C_Subtext
        Write-Host $($s.Safe) -ForegroundColor $C_Info
    }
}
Write-Host "└───────────────────────────────────────────────────────────────────┘" -ForegroundColor $C_Section
...
function Check-EventLog {
    ...
    if ($event) {
        Write-Host "  $message at: " -NoNewline -ForegroundColor $C_Subtext
        Write-Host $event.TimeCreated.ToString("MM/dd HH:mm") -ForegroundColor $C_Warn
    } else {
        Write-Host "  $message - No records found" -ForegroundColor $C_Info
    }
}
...
function Check-RecentEventLog {
    ...
    if ($event) {
        Write-Host "  $message (ID: $($event.Id)) at: " -NoNewline -ForegroundColor $C_Subtext
        Write-Host $event.TimeCreated.ToString("MM/dd HH:mm") -ForegroundColor $C_Warn
    } else {
        Write-Host "  $message - No records found" -ForegroundColor $C_Info
    }
...
function Check-DeviceDeleted {
    try {
        ...
        if ($event) {
            Write-Host "  Device configuration changed at: " -NoNewline -ForegroundColor $C_Subtext
            Write-Host $event.TimeCreated.ToString("MM/dd HH:mm") -ForegroundColor $C_Warn
            return
        }
    } catch {}
    ...
    try {
        ...
        if ($event) {
            Write-Host "  Device removed at: " -NoNewline -ForegroundColor $C_Subtext
            Write-Host $event.TimeCreated.ToString("MM/dd HH:mm") -ForegroundColor $C_Warn
            return
        }
    } catch {}
    ...
    try {
        ...
        if ($events) {
            Write-Host "  Last device change at: " -NoNewline -ForegroundColor $C_Subtext
            Write-Host $events.TimeCreated.ToString("MM/dd HH:mm") -ForegroundColor $C_Warn
            return
        }
    } catch {}

    Write-Host "  Device changes - No records found" -ForegroundColor $C_Info
}
...
if (Test-Path $prefetchPath) {
    Write-Host ""
    Write-Host "┌─ PREFETCH INTEGRITY ─────────────────────────────────────────────┐" -ForegroundColor $C_Section

    $files = Get-ChildItem -Path $prefetchPath -Filter *.pf -Force -ErrorAction SilentlyContinue
    if (-not $files) {
        Write-Host "  No prefetch files found. Please check the folder." -ForegroundColor $C_Warn
    } else {
        $hashTable = @{}
        $suspiciousFiles = @{}
        ...
        if ($hiddenAndReadOnlyFiles.Count -gt 0) {
            Write-Host ("  Hidden & Read-only Files: {0} found" -f $hiddenAndReadOnlyFiles.Count) -ForegroundColor $C_Warn
            foreach ($file in $hiddenAndReadOnlyFiles) {
                Write-Host ("    {0}" -f $file.Name) -ForegroundColor $C_Subtext
            }
        }

        if ($hiddenFiles.Count -gt 0) {
            Write-Host ("  Hidden Files: {0} found" -f $hiddenFiles.Count) -ForegroundColor $C_Warn
            foreach ($file in $hiddenFiles) {
                Write-Host ("    {0}" -f $file.Name) -ForegroundColor $C_Subtext
            }
        } else {
            Write-Host "  Hidden Files: None" -ForegroundColor $C_Info
        }

        if ($readOnlyFiles.Count -gt 0) {
            Write-Host ("  Read-Only Files: {0}" -f $readOnlyFiles.Count) -ForegroundColor $C_Warn
            foreach ($file in $readOnlyFiles) {
                Write-Host ("    {0}" -f $file.Name) -ForegroundColor $C_Subtext
            }
        } else {
            Write-Host "  Read-Only Files: None" -ForegroundColor $C_Info
        }

        if ($invalidSignatureFiles.Count -gt 0) {
            Write-Host ("  Invalid Signatures: {0}" -f $invalidSignatureFiles.Count) -ForegroundColor $C_Warn
            foreach ($file in $invalidSignatureFiles) {
                Write-Host ("    {0}" -f $file.Name) -ForegroundColor $C_Subtext
            }
        } else {
            Write-Host "  File Signatures: All good" -ForegroundColor $C_Info
        }

        $repeatedHashes = $hashTable.GetEnumerator() | Where-Object { $_.Value.Count -gt 1 }
        if ($repeatedHashes) {
            Write-Host ("  Duplicate Files: {0} sets found" -f $repeatedHashes.Count) -ForegroundColor $C_Warn
            foreach ($entry in $repeatedHashes) {
                ...
                Write-Host ("    Duplicate set: {0}" -f ($entry.Value -join ", ")) -ForegroundColor $C_Subtext
            }
        } else {
            Write-Host "  Duplicates: None" -ForegroundColor $C_Info
        }

        if ($suspiciousFiles.Count -gt 0) {
            Write-Host ""
            Write-Host ("  SUSPICIOUS FILES FOUND: {0}/{1}" -f $suspiciousFiles.Count, $totalFiles) -ForegroundColor $C_Warn
            foreach ($entry in $suspiciousFiles.GetEnumerator() | Sort-Object Key) {
                Write-Host ("    {0} : {1}" -f $entry.Key, $entry.Value) -ForegroundColor $C_Subtext
            }
        } else {
            Write-Host ""
            Write-Host ("  Prefetch integrity: Clean ({0} files checked)" -f $totalFiles) -ForegroundColor $C_Info
        }
    }
} else {
    Write-Host ""
...
try {
    ...
    Write-Host ""
    Write-Host "┌─ RECYCLE BIN ────────────────────────────────────────────────────┐" -ForegroundColor $C_Section
    ...
    if (Test-Path $recycleBinPath) {
        ...
        if ($userFolders) {
            ...
            Write-Host ("  Last Modified : {0}" -f $latestModTime.ToString("yyyy-MM-dd HH:mm:ss")) -ForegroundColor $C_Warn

            if ($allDeletedItems.Count -gt 0) {
                Write-Host ("  Total Items   : {0}" -f $allDeletedItems.Count) -ForegroundColor $C_Warn

                $latestItem = $allDeletedItems | Sort-Object LastWriteTime -Descending | Select-Object -First 1
                Write-Host ("  Latest Item   : {0}" -f $latestItem.Name) -ForegroundColor $C_Accent
            } else {
                Write-Host ("  Status        : Folders present but empty") -ForegroundColor $C_Info
            }
        } else {
            Write-Host ("  Status        : Empty") -ForegroundColor $C_Info
            Write-Host ("  Last Modified : {0}" -f $recycleBinFolder.LastWriteTime.ToString("yyyy-MM-dd HH:mm:ss")) -ForegroundColor $C_Info
        }

        $clearEvent = Get-WinEvent -FilterHashtable @{LogName="System"; Id=10006} -MaxEvents 1 -ErrorAction SilentlyContinue
        if ($clearEvent) {
            Write-Host ("  Last Cleared (Event) : {0}" -f $clearEvent.TimeCreated.ToString("yyyy-MM-dd HH:mm:ss")) -ForegroundColor $C_Error
        }
    } else {
        Write-Host ("  Recycle Bin not found at: {0}" -f $recycleBinPath) -ForegroundColor $C_Warn
        Write-Host "  Note: Recycle Bin may be empty or on a different drive" -ForegroundColor $C_Accent
    }
} catch {
